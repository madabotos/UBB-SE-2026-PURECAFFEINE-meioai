    ...
    public class RentalRepository : IRentalRepository
    {
        ...
        public void Add(Rental entity)
        {
            ...
            using var command = new SqlCommand(
                ...
                "VALUES(@game_id, @renter_id, @owner_id, @start_date, @end_date); SELECT SCOPE_IDENTITY();", connection);
            ...
            // [ENT-REN-01] Assign auto-generated ID
            entity.Id = Convert.ToInt32(command.ExecuteScalar());
        }

        public ImmutableList<Rental> GetRentalsByOwner(int ownerId) => GetByColumn("owner_id", ownerId);
        public ImmutableList<Rental> GetRentalsByRenter(int renterId) => GetByColumn("renter_id", renterId);
        public ImmutableList<Rental> GetRentalsByGame(int gameId) => GetByColumn("game_id", gameId);

        private ImmutableList<Rental> GetByColumn(string columnName, int value)
        {
            var list = new List<Rental>();
            using var connection = new SqlConnection(_connectionString);
            connection.Open();
            using var command = new SqlCommand($"SELECT * FROM Rentals WHERE {columnName} = @val", connection);
